...
namespace PE {
    ...
    public:
        ...
        __forceinline std::wstring GetName() const {
            std::wstring out{ };
            ...
            out = std::wstring(wide_name.begin(), wide_name.end());
    ...
    static __declspec(noinline) Module GetModule(std::wstring name) {
        modules_t modules;
    ...
    static __declspec(noinline) Address GetExport(const Module& module, const std::wstring& name);
    ...
    static __declspec(noinline) Address GetExport(const Module& module, const hash32_t name) {
        ...
        size_t      delim;
        std::wstring export_name, fwd_str, fwd_module, fwd_export;
        ...
        for (size_t i{ 0u }; i < dir->NumberOfNames; ++i) {
            export_name = Address::RVA<const wchar_t* >(base, names[i]);
            ...
            if (FNV1a::get(export_name) == name) {
                ...
                if (export_ptr >= (uintptr_t)dir && export_ptr < ((uintptr_t)dir + data_dir->Size)) {
                    ...
                    fwd_str = export_ptr.as< const wchar_t* >();
